...
data_iNatTools <- readRDS("../data/mtl_bee_iNatTools_2020-05-23.rds")

# Import data gathered directly from API
data_api <- readRDS("../data/mtl_bee_API_2020-05-23.rds")
...
~ 70% (296) of observations received *research* quality grade. Others (120) were identified as *needs_id*.

 ~80% (294) of observations identified as *most_agree* and *some_agree* and none of the *most_disagree* observations recieved *research* quality grade. The remaining (~20% observations identified as *most_agree* and *some_agree* and 100% of *most_disagree* observations) recieved a *needs_id* quality grade.
...
```{r Bees canada}
Bees_canada <- read.csv("../data/Bees_canada_26_05_20.csv")
```

The dataset *Bees_canada* is comprized of 360009 records and 34 columns/variables.
...
```{r Quality grade}
ggplot(data=Bees_canada, aes(x= Bees_canada$scientific_name, fill=Bees_canada$quality_grade))+
...
There are too many taxa to plot! 
...
Bees_CA_Qgrade <- plyr::count(Bees_canada, c("scientific_name","quality_grade"))
...
# Plot
ggplot(data=Bees_Needs_ID_long, aes(x= Bees_Needs_ID_long$scientific_name, y=Bees_Needs_ID_long$value, fill=Bees_Needs_ID_long$variable))+

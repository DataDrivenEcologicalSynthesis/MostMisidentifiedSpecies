...
hist(RatioMisIdSup0$RatioMisId,
...
bee_color_palette <- c('#f8f7de','#f7ea87'	,'#d29d30'	,'#854d0d'	,'#3a200c')
# Color palette
...
pie(slices,labels = lbls, cex=1.5,col=bee_color_palette, main=title("More than 2/3 of the taxa disagreed upon \n are identified to species level", line = -2,cex.main=2))
```
...
Taxonomy<- plyr::count(RatioMisIdSup0, c("taxon.rank","taxon.name"))
...
Genus <-  ggplot(data=(Taxonomy[Taxonomy$taxon.rank=="genus",]), aes(x = taxon.rank ,y = freq, fill = taxon.name))+
  ...
  theme(axis.title.x=element_blank(),
        ...
        legend.position = "bottom")+
  scale_fill_viridis("Genus",discrete=FALSE)+
  ylab("Count")
...
Species <-  ggplot(data=(Taxonomy[Taxonomy$taxon.rank=="species",]), aes(x = taxon.rank ,y = freq, fill = taxon.name))+
   geom_bar(stat="identity", position="dodge")+
   theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
        legend.position = "bottom")+
  scale_fill_viridis("Species",discrete=FALSE)+
  ylab("Count")
```

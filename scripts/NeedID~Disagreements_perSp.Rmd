...
ggplot(full_data2,aes(x=MisIDRatio,y=NeedsIDRatio,label=paste(taxon.name,"\n",identifications_count)))+
  geom_point(aes(colour=identifications_count))+
  geom_text(check_overlap = T)+
  scale_color_gradient(name="Identifications count", low='#fdb863', high='#5e3c99')+
  ggtitle("Both disagreement ratio and needs ID ratio seem to increase when the species have not been frequently identified.")+
  xlab("Disagreement ratio per species")+
  ylab("Needs ID ratio per species")+
  theme( axis.title.y = element_text(size=15),
         axis.text.y= element_text(size=13),
         axis.title.x = element_text(size=15),
         axis.text.x= element_text(size=13))
